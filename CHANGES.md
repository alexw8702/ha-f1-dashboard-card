...
- **Active Aero Zonen jetzt real auf der Streckenkarte eingezeichnet** (nicht nur als Zahl im Badge) — die Zonenzahl allein half wenig, wenn man die Zonen nicht sieht. Positionen wurden pro Strecke recherchiert (offizielle FIA-/Streckenguide-Angaben) und als teal Linie direkt auf der weißen Outline gezeichnet: Spa (Kemmel-Gerade + Boxengerade), Miami (alle 3: Rückgerade, Boxengerade, Mittelsektor-Gerade), Suzuka (Boxengerade zu Turn 1), Villeneuve (2 von 3 — die kürzeste Zone ließ sich aus der reinen Umriss-Geometrie nicht sicher genug lokalisieren, daher bewusst weggelassen statt geraten). Weitere Strecken folgen schrittweise. Badge mit der offiziellen Zonenzahl bleibt zusätzlich bestehen (bisher "DRS-Zonen" genannt, Umbenennung ab dieser Saison).
